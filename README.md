...
| `jobber release [patch\|minor\|major\|x.y.z]` | Bump версии + git-тег + push (+ публикация в npm при `NPM_TOKEN`). |
...
Из исходного git-клона одной командой:

```bash
NPM_TOKEN=<токен-с-bypass-2FA> jobber release patch   # 1.0.0 → 1.0.1: bump + tag + push + publish
# без NPM_TOKEN — только bump+tag+push, публикацию сделает GitHub Actions по тегу
jobber release minor          # 1.0.x → 1.1.0
jobber release 2.0.0          # явная версия
```

Команда синхронизирует версию в `package.json` и `pyproject.toml`, коммитит `release: vX.Y.Z`,
ставит тег и пушит. Выберите **один** способ публикации (локально через `NPM_TOKEN` **или** CI-секрет),
иначе вторая попытка упадёт на дубликате версии.
